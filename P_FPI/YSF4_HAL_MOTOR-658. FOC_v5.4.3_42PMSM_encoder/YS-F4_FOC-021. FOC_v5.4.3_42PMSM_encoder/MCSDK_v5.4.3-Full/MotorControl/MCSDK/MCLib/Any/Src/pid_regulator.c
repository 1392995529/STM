...
__weak void FUZZYPID_HandleInit( PID_Handle_t * pHandle )
{
  pHandle->hKpGain =  pHandle->hDefKpGain;
  pHandle->hKiGain =  pHandle->hDefKiGain;
  pHandle->hKdGain =  pHandle->hDefKdGain;
  pHandle->hKe =  pHandle->hDefKe;
  pHandle->hKec =  pHandle->hDefKec;
  pHandle->hKup =  pHandle->hDefKup;
  pHandle->hKui =  pHandle->hDefKui;
  pHandle->wIntegralTerm = 0x00000000UL;
  pHandle->wPrevProcessVarError = 0x00000000UL;
}
/**
...
void PID_SetKE( PID_Handle_t * pHandle, float hKe )
{
  pHandle->hKe = hKe;
}

 void PID_SetKEC( PID_Handle_t * pHandle, float hKec )
{
  pHandle->hKec = hKec;
}

 void PID_SetKUP( PID_Handle_t * pHandle, int16_t hKup )
{
  pHandle->hKup = hKup;
}

 void PID_SetKUI( PID_Handle_t * pHandle, int16_t hKui )
{
  pHandle->hKui = hKui;
}
/**
...
//速度环变论域解模糊
 void SpeedFuzzyPIComputation (PID_Handle_t * pHandle,int32_t wProcessVarError)
{
 float qValue[2]={0,0};  
 ...
 int32_t thisError;
 int32_t deltaError;
 float alphae ,alphaec ,betae;  //定义伸缩因子
 
//量化
 thisError = wProcessVarError;
 deltaError = wProcessVarError - pHandle->wPrevProcessVarError;

// qValue[0]=pHandle->hKe*thisError/pHandle->alphae;
// qValue[1]=pHandle->hKec*deltaError/pHandle->alphaec;
 qValue[0]=pHandle->hKe*thisError;
 qValue[1]=pHandle->hKec*deltaError;
 betae = 1.7-exp(-pow((thisError-300),2)/10000);
 if (qValue[0] > 6.0F) {
    qValue[0] = 6.0F;
  }
 else {
...
//计算ec的隶属度和索引
 ...
 //采用重心法计算pid增量值
 pidvalue[0]= (int16_t)(msE[0]*(msEC[0]*speedruleKp[indexEC[0]][indexE[0]]+msEC[1]*speedruleKp[indexEC[0]][indexE[1]]) 
            +msE[1]*(msEC[0]*speedruleKp[indexEC[1]][indexE[0]]+msEC[1]*speedruleKp[indexEC[1]][indexE[1]]));
 pidvalue[1]= (int16_t)(msE[0]*(msEC[0]* speedruleKi[indexEC[0]][indexE[0]]+msEC[1]*speedruleKi[indexEC[0]][indexE[1]])
            +msE[1]*(msEC[0]*speedruleKi[indexEC[1]][indexE[0]]+msEC[1]*speedruleKi[indexEC[1]][indexE[1]]));

// pidvalue[0]= (int16_t)(msE[0]*(msEC[0]*speedruleKp[indexE[0]][indexEC[0]]+msEC[1]*speedruleKp[indexE[0]][indexEC[1]]) 
//            +msE[1]*(msEC[0]*speedruleKp[indexE[1]][indexEC[0]]+msEC[1]*speedruleKp[indexE[1]][indexEC[1]]));
// pidvalue[1]= (int16_t)(msE[0]*(msEC[0]* speedruleKi[indexE[0]][indexEC[0]]+msEC[1]*speedruleKi[indexE[0]][indexEC[1]])
//            +msE[1]*(msEC[0]*speedruleKi[indexE[1]][indexEC[0]]+msEC[1]*speedruleKi[indexE[1]][indexEC[1]]));


//pi增量修正
// pHandle->deta_Kp=pHandle->hKup*(int16_t)betae*pidvalue[0];
// pHandle->deta_Ki=pHandle->hKui*(int16_t)betae*pidvalue[1];
 pHandle->deta_Kp=pHandle->hKup*pidvalue[0];
 pHandle->deta_Ki=pHandle->hKui*pidvalue[1];

 alphae = 0.5*pow((qValue[0]/6),0.8)+0.5;
 alphaec = 0.5*pow((qValue[1]/6),0.8)+0.5;
 pHandle->alphae = alphae;
 pHandle->alphaec = alphaec;
}


///////////////////////////////////////速度环模糊PI//////////////////////////////////////////////////////
__weak int16_t FUZZYPI_Controller( PID_Handle_t * pHandle, int32_t wProcessVarError ) 
{
  int32_t wProportional_Term, wIntegral_Term, wOutput_32, wIntegral_sum_temp; //比例 积分  输出 积分和
  int32_t wDischarge = 0;
  int16_t hUpperOutputLimit = pHandle->hUpperOutputLimit;  //输出限幅
  int16_t hLowerOutputLimit = pHandle->hLowerOutputLimit;
  
  SpeedFuzzyPIComputation(pHandle,wProcessVarError); //解模糊得到△kp，△ki
   pHandle->wPrevProcessVarError = wProcessVarError;
  /* Proportional term computation*/
  wProportional_Term = (pHandle->hKpGain + pHandle->deta_Kp) * wProcessVarError;  //比例项计算

  /* Integral term computation */
  if ( pHandle->hKiGain == 0 )                   //积分项计算
  {
    pHandle->wIntegralTerm = 0;
  }
  else
  {
    wIntegral_Term = (pHandle->hKiGain+ pHandle->deta_Ki) * wProcessVarError;
    wIntegral_sum_temp = pHandle->wIntegralTerm + wIntegral_Term;
    ...
    if ( wIntegral_sum_temp < 0 )
    {
      if ( pHandle->wIntegralTerm > 0 )
      {
        if ( wIntegral_Term > 0 )
        {
          wIntegral_sum_temp = INT32_MAX;          //求和值超过INT32范围溢出
        }
      }
    }
    else
    ...
    if ( wIntegral_sum_temp > pHandle->wUpperIntegralLimit )   //限制积分项范围
    {
      pHandle->wIntegralTerm = pHandle->wUpperIntegralLimit;
    }
    else if ( wIntegral_sum_temp < pHandle->wLowerIntegralLimit )
