...
//                  Position_GetHandwhellpulse();
       ...
       if(Position_M1.Mode_Flag == P_SPEED_MODE)
        {
            // 这里会调用 mc_position.c 中新写的 Position_CalcSpeedReferrence
            // 其中已经包含了 PI + Feedforward 的求和逻辑
            Position_M1.Set_Speed = Position_CalcSpeedReferrence();
            
            // 下发速度指令，参数2设为0 (无斜坡)，保证前馈的响应速度
  ...
  if ((Currentspeed *Targetspeed< 0)&&(Abs_Value(Targetspeed- Currentspeed)> 500))
    {
                  STM_NextState( &STM[M1], ANY_STOP );
