...
/**
  ...
  *         Current(digit) = [Current(Amp) * 65536 * Rshunt * Aop]  /  Vdd micro
  */
__weak int16_t STC_CalcTorqueReference( SpeednTorqCtrl_Handle_t * pHandle )   //计算扭矩参考值
{
  ...
  if ( pHandle->Mode == STC_SPEED_MODE )
  {
    ...
    hTorqueReference = FUZZYPI_Controller( pHandle->PISpeed, ( int32_t )hError );  //速度环FUZZYPI，
