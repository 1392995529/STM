...
int16_t detaEncX = 0;
int16_t detaEncY = 0;
volatile uint8_t AD_Data_Ready_Flag = 0;
...
/* 定义广播发送函数 */
void CAN_Send_Sync_Broadcast(void)
{
  hCAN.pTxMsg->ExtId = 0x1314;       /* 统一使用扩展ID 0x1314 */
  hCAN.pTxMsg->IDE = CAN_ID_EXT;     /* 扩展模式 */
  hCAN.pTxMsg->RTR = CAN_RTR_DATA;   /* 发送的是数据 */
  hCAN.pTxMsg->DLC = 4;              /* 数据长度为4字节 */

  /* 前两个字节放 DB_data[0] -> 给从板1 */
  hCAN.pTxMsg->Data[0] = DB_data[0] & 0xFF;
  hCAN.pTxMsg->Data[1] = DB_data[0] >> 8;

  /* 后两个字节放 DB_data[1] -> 给从板2 */
  hCAN.pTxMsg->Data[2] = DB_data[1] & 0xFF;
  hCAN.pTxMsg->Data[3] = DB_data[1] >> 8;
}
...
int main(void)
{
  ...
  while (1)
  {
 if(AD_Data_Ready_Flag == 1)
    {
        // 仅仅是调用，没有 void，没有大括号内容
        CAN_Send_Sync_Broadcast(); 
 
        HAL_CAN_Transmit(&hCAN, 10);
        AD_Data_Ready_Flag = 0;
    }
  }
  /* USER CODE END 3 */
}  // 
...
//       MC_StartMotor1();
  ...
  else if(GPIO_Pin==KEY4_GPIO_PIN)
  {
    ...
    {
//      Position_SetAngle(20); //，819200对应减速后四分之一圈 1degree=（360/65536）°
//      MC_StartMotor1();
      PositionMode = false;
    //  DK_data[3]=1;
    }
...
/* --- 修改处：不要在这里发CAN，只立个Flag --- */
      AD_Data_Ready_Flag = 1;
      /* ---------------------------------------- */
   }
//      CAN_SetTxMsg();
...
//          {
//
//          case 0:{AD_data0[times] = DB_data[0];}
//          break;
//          
//          case 1:{AD_data1[times] = DB_data[0];}
//          break;
//          
//          case 2:{AD_data2[times] = DB_data[0];}
//          break;
//          
//          case 3:{AD_data3[times] = DB_data[0];}
//          break;
//          
//          case 4:{AD_data4[times] = DB_data[0];}
//          break;
//          
//          case 5:{AD_data5[times] = DB_data[0];}
//          break;
//          
//          }
//      
