...
PID_Handle_t *pPIDSpeed[NBR_OF_MOTORS]; //定义PID结构体指针
PID_Handle_t *pPIDIq[NBR_OF_MOTORS];
PID_Handle_t *pPIDId[NBR_OF_MOTORS];
FeedForward_Handle_t *FFPosition; //位置环相关结构体指针
Position_Handle_t *pPIDPosition;
Observer_Handle_t *TorqueObserver;  //转矩观测器结构体指针
RLS_Handle_t *pRLS;   //参数辨识结构体指针
...
bool ControlMode = true;    //控制模式选择  1：位置模式   0：速度模式
bool  PositionMode = true;   //位置模式选择  1.实际位置    0：PSD追踪模式
...
/**
  ...
  * @retval None
  */
__weak void MCboot( MCI_Handle_t* pMCIList[NBR_OF_MOTORS],MCT_Handle_t* pMCTList[NBR_OF_MOTORS] ) //电机启动函数，启动FOC控制功能
{
  ...
  /******************************************************/
//  PID_HandleInit(&PIDSpeedHandle_M1);    //速度环PID初始化
//  pPIDSpeed[M1] = &PIDSpeedHandle_M1;
  FUZZYPID_HandleInit(&FUZZYPIDSpeedHandle_M1);    //速度环FUZZYPID参数初始化
  pPIDSpeed[M1] = &FUZZYPIDSpeedHandle_M1;  //解引用
  ...
  /********************************************************/
  PID_HandleInit(&PIDIqHandle_M1);    //电流环Iq PID初始化
  PID_HandleInit(&PIDIdHandle_M1);    //电流环Id PID初始化
  pPIDIq[M1] = &PIDIqHandle_M1;
  pPIDId[M1] = &PIDIdHandle_M1;
  FFPosition = &FF_M1;
  TorqueObserver = &Torque_observation; 
  pPIDPosition = &Position_M1;
  pRLS = &RLS;
  /********************************************************/
...
/**
  ...
  * are executed here.
  */
__weak void TSK_MediumFrequencyTaskM1(void)   //中频任务，包含速度环
{
  ...
  case IDLE_START:
    ...
    if ( EAC_IsAligned( &EncAlignCtrlM1 ) == false )   //若编码器未对齐
    {
      ...
      STM_NextState( &STM[M1], IDLE_ALIGNMENT );    //切换至对齐编码器状态机
      break;
    }
...
/* Get error position/Angle, unit in rad*10000 */
    if (ControlMode)
    {
               if (PositionMode)
               {
                  Position_GetErrorAngle();
               }
               else
               {
                 Position_GetErrorVoltage();
               }	        
...
//		if((Position_M1.Speed_ACC_Count > SPEED_ACC_RATE) && (Position_M1.Mode_Flag == P_SPEED_MODE))
                if(Position_M1.Mode_Flag == P_SPEED_MODE)
		{
			...
			Position_M1.Control_Speed = MC_GetLastRampFinalSpeedMotor1(); // 设定转速
			Position_M1.Real_Speed = MC_GetMecSpeedAverageMotor1();	// 实际平均转速
...
//			MC_ProgramSpeedRampMotor1(Position_M1.Set_Speed,Position_M1.Duration_ms);
                      MC_ProgramSpeedRampMotor1(Position_M1.Set_Speed,0);
		}
    ...
    MCI_ExecBufferedCommands( oMCInterface[M1] );  //执行缓存中的指令，电机运行的时候才会被调用
    FOC_CalcCurrRef( M1 );  //计算电流参考值
    Currentspeed = MC_GetLastRampFinalSpeedMotor1();
    Targetspeed = MC_GetMecSpeedAverageMotor1();
    if ((Currentspeed *Targetspeed< 0)&&(Abs_Value(Targetspeed- Currentspeed)> 300))
    {
                  STM_NextState( &STM[M1], ANY_STOP );
                  EAC_SetRestartState( &EncAlignCtrlM1,true );    //设置为开启重新启动flag
                  Set_Encoder_notAligned(&EncAlignCtrlM1);        //设置为编码器未对齐flag
                  break;
    }
...
/**
  ...
  * @retval none
  */
__weak void FOC_CalcCurrRef(uint8_t bMotor)  //计算电流参考值（Iq）
{
...
inline uint16_t FOC_CurrControllerM1(void)
{
  ...
  Vqd = Circle_Limitation(pCLM[M1], Vqd);       //限幅
...
//  Parameter_identification(pRLS);
//  Luenberge_GetTl(TorqueObserver);   //观测转矩
  FOCVars[M1].Vqd = Vqd;
  FOCVars[M1].Iab = Iab;
...
int32_t Position_GetMecAngle(void)  //机械角度
{
  int32_t wAngle;
  SpeednPosFdbk_Handle_t *speedHandle;
  speedHandle = STC_GetSpeedSensor(pSTC[M1]);
  wAngle = SPD_GetMecAngle(speedHandle);  
  return  wAngle;
}
/**
