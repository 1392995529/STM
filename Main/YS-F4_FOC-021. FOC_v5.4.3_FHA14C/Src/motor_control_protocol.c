...
__weak void MCP_ReceivedFrame(MCP_Handle_t *pHandle, uint8_t Code, uint8_t *buffer, uint8_t Size)
{
  ...
  switch (Code)
  {
  case MC_PROTOCOL_CODE_SET_REG:        /////////////写寄存器/////////
    {
      ...
      case MC_PROTOCOL_REG_SPEED_KUP:
      case MC_PROTOCOL_REG_SPEED_KUI:
      case MC_PROTOCOL_REG_POSITION_KUP:
      case MC_PROTOCOL_REG_POSITION_KUI:
        {
          /* 16bit variables */
      ...
      case MC_PROTOCOL_REG_TARGET_POSITION:
        ...
        break;
      case MC_PROTOCOL_REG_POSITION_KVFF:     //////////////前馈参数KVFF/////////
      case MC_PROTOCOL_REG_POSITION_KAFF:     //////////////前馈参数KAFF/////////
      case MC_PROTOCOL_REG_SPEED_KE:          /////////速度环模糊参数KE//////////
      case MC_PROTOCOL_REG_SPEED_KEC:         /////////速度环模糊参数KEC/////////
      case MC_PROTOCOL_REG_POSITION_KE:       /////////位置环模糊参数KE//////////
      case MC_PROTOCOL_REG_POSITION_KEC:       /////////位置环模糊参数KE//////////
        {
          FloatToU32 PositionKvff;
          PositionKvff.U32_Val = buffer[1] + (buffer[2] << 8) + (buffer[3] << 16) + (buffer[4] << 24);
          bNoError = UI_SetRegFloat(bRegID , PositionKvff.Float_Val);
        }
        break;       
  ...
  case MC_PROTOCOL_CODE_GET_REG:       ///////////////读寄存器/////////////////
    {
      MC_Protocol_REG_t bRegID = (MC_Protocol_REG_t)buffer[0];
  ...
  case MC_PROTOCOL_CODE_EXECUTE_CMD:    //指令命令
    {
  ...
  case MC_PROTOCOL_CODE_GET_REVUP_DATA:           //更改成包含多个信息的数据
    {
        uint8_t outBuff[20];
        int32_t speedmeas,speedref,posref,posmeas,voltage;
        speedref = (int32_t)MC_GetLastRampFinalSpeedMotor1();
        speedmeas = (int32_t)MC_GetMecSpeedAverageMotor1();
        posref = (int32_t)Position_GetTargrtangle();
        posmeas = (int32_t)Position_GetAngle();
        voltage =  (int32_t)DB_data[0];
        outBuff[0] = (uint8_t) (speedref);
        outBuff[1] = (uint8_t)((speedref) >> 8);
        outBuff[2] = (uint8_t)((speedref) >> 16);
        outBuff[3] = (uint8_t)((speedref) >> 24);
        outBuff[4] = (uint8_t) (speedmeas);
        outBuff[5] = (uint8_t)((speedmeas) >> 8);
        outBuff[6] = (uint8_t)((speedmeas) >> 16);
        outBuff[7] = (uint8_t)((speedmeas) >> 24);
        outBuff[8] = (uint8_t)(posref);
        outBuff[9] = (uint8_t)(posref >> 8);
        outBuff[10] = (uint8_t)(posref >> 16);
        outBuff[11] = (uint8_t)(posref >> 24);
        outBuff[12] = (uint8_t)(posmeas);
        outBuff[13] = (uint8_t)(posmeas >> 8);
        outBuff[14] = (uint8_t)(posmeas >> 16);
        outBuff[15] = (uint8_t)(posmeas >> 24);
        outBuff[16] = (uint8_t)(voltage);
        outBuff[17] = (uint8_t)(voltage >> 8);
        outBuff[18] = (uint8_t)(voltage >> 16);
        outBuff[19] = (uint8_t)(voltage >> 24);
        pHandle->fFcpSend(pHandle->pFCP, ACK_NOERROR, outBuff, 20);
...
//      pHandle->fFcpSend(pHandle->pFCP, ACK_NOERROR, outBuff, 8 );
  ...
  case MC_PROTOCOL_CODE_SET_REVUP_DATA:    //更改为设置模式
    {
        int16_t state;
        state = buffer[0] + (buffer[1] << 8);
        bNoError =  UI_SetControlModel(state);
