...
typedef struct MCP_Handle_s
{
  ...
  FCP_Handle_t *pFCP;
  FCP_SendFct_t fFcpSend;     //帧通讯协议发送函数，函数指针
  FCP_ReceiveFct_t fFcpReceive;   //帧通讯协议接收函数，函数指针
  FCP_AbortReceiveFct_t fFcpAbortReceive;  //帧通讯协议终止函数，函数指针  （指向函数的指针）
  uint8_t BufferFrame[FCP_MAX_PAYLOAD_SIZE];  /**< buffer containing data */ //通讯帧缓存数组128
  const char *s_fwVer;    /**< String of FW version used */
  uint8_t BufferSize;      /**< Frame buffer size */  //帧缓存大小

} MCP_Handle_t;  //通讯协议结构体
