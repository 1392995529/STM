...
/**
  ...
  * @retval Return the currently selected MC, zero based, on which UI operates.
  */
__weak bool UI_SetRegFloat(MC_Protocol_REG_t bRegID, float wValue)  //加入写float类型参数接口函数
{
  bool retVal = true;
  switch (bRegID)
  {
  case MC_PROTOCOL_REG_POSITION_KVFF:            //速度前馈参数
    {
      Position_SetKvff(FFPosition,wValue);
    }
   break;
   
  case MC_PROTOCOL_REG_POSITION_KAFF:            //加速度前馈参数
    {
      Position_SetKaff(FFPosition,wValue);
    }
   break;
  case MC_PROTOCOL_REG_SPEED_KE:                 //速度量化因子KE
    {
      PID_SetKE(pPIDSpeed[0],wValue);
    }
   break;
   
   case MC_PROTOCOL_REG_SPEED_KEC:               //速度量化因子KEC
    {
      PID_SetKEC(pPIDSpeed[0],wValue);
    }
   break;
   
   case MC_PROTOCOL_REG_POSITION_KE:             //位置量化因子KE
    {
      Position_SetKE(pPIDPosition,wValue);
    }
   break;
   
   case MC_PROTOCOL_REG_POSITION_KEC:            //位置量化因子KEC
    {
      Position_SetKEC(pPIDPosition,wValue);
    }
   break;
...
__weak bool UI_SetReg(UI_Handle_t *pHandle, MC_Protocol_REG_t bRegID, int32_t wValue)  
{
  ...
  case MC_PROTOCOL_REG_POSITION_KP: //设定位置kp  
    {
      Position_SetKP(pPIDPosition, (int16_t)wValue);
    }
   break;
    
  case MC_PROTOCOL_REG_POSITION_KI: //设定位置Ki 
    {
      Position_SetKI(pPIDPosition, (int16_t)wValue);
    }
   break;
   
   case MC_PROTOCOL_REG_POSITION_KUP:  //设定位置Kup 
    {
      Position_SetKUP(pPIDPosition,(int16_t)wValue);
    }
   break;
   
   case MC_PROTOCOL_REG_POSITION_KUI:  //设定位置Kui 
    {
      Position_SetKUI(pPIDPosition,(int16_t)wValue);
    }
   break;
   
  case MC_PROTOCOL_REG_TARGET_POSITION:  //设定位置目标
    {
      Position_SetAngle(wValue);
    }
    break;
    
  case MC_PROTOCOL_REG_SPEED_KUP:  //设定速度环KUP
    {
      PID_SetKUP(pPIDSpeed[0],(int16_t)wValue);
    }
    break;
    
  case MC_PROTOCOL_REG_SPEED_KUI:  //设定速度环KUI
    {
      PID_SetKUI(pPIDSpeed[0],(int16_t)wValue);
    }
   break;
...
bool UI_SetControlModel(int16_t state)
        {
          bool retVal = true;
          switch (state)
          {
...
__weak int32_t UI_GetReg(UI_Handle_t *pHandle, MC_Protocol_REG_t bRegID, bool * success)
{
  ...
  switch (bRegID)
  {
    ...
    case MC_PROTOCOL_REG_POSITION_KP: //读取位置kp  
    {
      bRetVal = (int32_t)Position_GetKP(pPIDPosition);
    }
    break;  
    
    case MC_PROTOCOL_REG_CURRENT_POSITION:  //读取当前位置  （需进行转换）
    {
     bRetVal = (int32_t)Position_GetAngle();
    }
      
    break;  
    
     case MC_PROTOCOL_REG_TARGET_POSITION:  //读取目标位置
     {
       bRetVal = (int32_t)Position_GetTargrtangle();
     }
