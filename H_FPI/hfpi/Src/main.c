...
void HAL_CAN_RxCpltCallback(CAN_HandleTypeDef *hcan)
{
    ...
    {
        /* 2. 从板2 读取后两个字节：Data[2] 和 Data[3] */
        /* 这对应主板发出的 DB_data[1] */
        /* 将其合成 16位整数，存入本地 DB_data[0] 供电机控制使用 */
      DB_data[0] = (int16_t)(hcan->pRxMsg->Data[1] << 8 | hcan->pRxMsg->Data[0]);

}
/* 3. 重新开启接收中断，防止只收一次就停了 */
    HAL_CAN_Receive_IT(hcan, CAN_FIFO0);
}
